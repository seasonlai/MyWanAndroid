...
public class CustomLayoutManager extends RecyclerView.LayoutManager {
  ...
  private void calculateChildrenSite(RecyclerView.Recycler recycler) {
    ...
    for (int i = 0; i < getItemCount(); i++) {
      ...
      addView(view);
      // 我们自己指定ItemView的尺寸。
      measureChildWithMargins(view, ContextUtils.getSreenWidth(MyApp.getContext()) / 2, 0);
      ...
      if (i % 2 == 0) { // 当i能被2整除时，是左，否则是右。
        // 左
        mTmpRect.set(0, totalHeight, ContextUtils.getSreenWidth(MyApp.getContext()) / 2, totalHeight + height);
      } else {
        // 右，需要换行
        mTmpRect.set(ContextUtils.getSreenWidth(MyApp.getContext())  / 2, totalHeight, ContextUtils.getSreenWidth(MyApp.getContext()) ,
            totalHeight + height);
  ...
  private void recycleAndFillView(RecyclerView.Recycler recycler, RecyclerView.State state) {
    ...
    for (int i = 0; i < getItemCount(); i++) {
      ...
      if (Rect.intersects(displayRect, allItemRects.get(i))) {
        ...
        measureChildWithMargins(itemView, ContextUtils.getSreenWidth(MyApp.getContext()) / 2, 0);
